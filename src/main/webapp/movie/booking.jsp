...
<head>
 ...
 <script  type="text/javascript">
	...
	document.addEventListener('DOMContentLoaded', function() {
	     ...
	     movieItems.forEach(item => {
	       item.addEventListener('click', function() {
	         const mcode = this.getAttribute('data-mcode');   //영화코드
	         setMcode(mcode);
	         highlightMovie(this);
	       });
	       ...
	       item.addEventListener('mouseleave', function() {
	         if (!this.classList.contains('selectedMovie')) {
	...
	function highlightMovie(element) {
	     const selectedElements = document.querySelectorAll('.selectedMovie');
	     selectedElements.forEach(selectedElement => {
	       selectedElement.classList.remove('selectedMovie');
	       selectedElement.style.backgroundColor = '';
	     });

	     element.classList.add('selectedMovie');
	   }
	...
	document.addEventListener('DOMContentLoaded', function() {
	  ...
	  seats.forEach(seat => {
	    ...
	    if(seat.classList.contains('booked')) {
          ...
          const selectedSeatCount = selectedSeats.length * 10000;
	  ...
	  document.getElementById('booking').addEventListener('click', () => {
	    ...
	    const selectedPrice = document.getElementById('ticketPrice').textContent;
	    ...
	    if(!mcode) {
	    	...
	    	reserveSeats(selectedSeatNumbers, selectedDate, mcode, selectedPrice);
	    }
	    
	  });
	  
	  // 선택된 좌석 예약 함수
	  function reserveSeats(selectedSeatNumbers, selectedDate, mcode, selectedPrice) {
	  var message = "";
		  if (selectedSeatNumbers.length > 0) {
		    ...
		    //check.jsp로 값넘김
		    var msg="bookingseat_check.jsp?seat="+selectedSeatNumbers+ "&date=" + selectedDate + "&mcode=" + mcode + "&price=" + selectedPrice; //check에서 seat로 값넘김
	  ...
	  document.getElementById('seatStatus').addEventListener('click', () => {
	    const selectedDate = document.getElementById('selectedDate').textContent;
	    ...
	    if(!mcode) {
	    	...
	    	reserveSeats2(selectedDate, mcode);
	    }
	    
	  });
	  
	  // 영화코드를 bookingseat_search로 보냄
	  function reserveSeats2(selectedDate, mcode) {
		    //search.jsp로 값넘김
		    var msg="bookingseat_search.jsp?date=" + selectedDate + "&mcode=" + mcode; 
		    document.formDate.action=msg;
		    document.formDate.method="post";
...
%>      
  ...
  <div id="headerc">
    ...
    <div id="dateSelect">
    	<form name="formDate" id="formDate" method="post" action="booking.jsp">
